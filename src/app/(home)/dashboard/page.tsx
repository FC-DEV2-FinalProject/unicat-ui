      ...
      <Card className="w-full bg-white rounded-2xl border border-solid border-gray-1 shadow-[0px_1px_8px_2px_#0000000a]">
        <div className="flex w-full items-center gap-8 p-5 pb-0">
          <div className="font-med-20 text-base-900 text-[length:var(--med-20-font-size)] tracking-[var(--med-20-letter-spacing)] leading-[var(--med-20-line-height)]">
            {new Date(dashboardData.content[0]?.createdAt || "").toLocaleDateString("ko-KR", {
              year: "numeric",
              month: "2-digit",
              day: "2-digit",
            })}
        ...
        <CardContent className="flex w-full flex-wrap items-center justify-start gap-6 px-6 py-6">
          {dashboardData.content.map((project) => (
            <div key={project.id} className="relative w-full sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] h-[210px] min-w-[280px]">
              <div className="flex flex-col h-full bg-slate-50 rounded-xl overflow-hidden">
                <img className="w-full h-[140px] object-cover" alt="Project thumbnail" src={project.thumbnailUrl} />
                <div className="flex flex-col gap-2 p-4">
                  ...
                  <div className="font-normal text-gray-5 text-base text-center tracking-[0] leading-5 line-clamp-2">{project.subtitle}</div>
                </div>
              </div>
            </div>
          ))}
